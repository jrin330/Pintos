...
thread_awake(int64_t tick){
  ...
  while(e != list_end(&block_list)){
    struct thread *iter = list_entry(e, struct thread, elem);
    if(iter->blocked_time <= tick){
      e = list_remove(e);
      thread_unblock(iter);
    }
    else
...
thread_yield (void) 
{
  ...
  if (cur != idle_thread) 
    list_insert_ordered(&ready_list, &cur->elem, compare_priority_by_elem, NULL);
...
bool compare_priority_by_elem(const struct list_elem *l, const struct list_elem *e, void *aux UNUSED){
...
void thread_aging(void){
  if(thread_current() != idle_thread)
    thread_current()->recent_cpu = add_btw_diff(thread_current()->recent_cpu, 1);

  if(timer_ticks() % 4 == 0){
    thread_foreach(set_new_priority, NULL);

    if(timer_ticks() % TIMER_FREQ == 0){
      //get new load_avg and  recent_cpu
      fixed coeffi[2];
...
thread_set_nice (int nice UNUSED) 
{
  ...
  set_new_priority(thread_current(), NULL);
  preemption();
  intr_set_level(old_level);
}
...
thread_get_load_avg (void) 
{
  /* Not yet implemented. */
  enum intr_level old_level = intr_disable();
  int tmp = fixed_to_int(mult_btw_diff(load_avg, 100));
  intr_set_level(old_level);
  return tmp;
}
